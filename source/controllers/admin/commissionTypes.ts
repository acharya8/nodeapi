...
const getCommissionType = async (req: Request, res: Response, next: NextFunction) => {
    try {
        logging.info(NAMESPACE, 'Getting Commission Types');
        ...
        if (authorizationResult.statusCode == 200) {
            ...
            if (result && result.length > 0) {
                if (result[0] && result[0].length > 0) {
                    let successResult = new ResultSuccess(200, true, 'Get Commission Type Successfully', result[0], result[0].length);
                    return res.status(200).send(successResult);
                } else if (result[0] && result[0].length == 0) {
                    let successResult = new ResultSuccess(200, true, 'Get Commission Type Successfully', [], result[0].length);
                    return res.status(200).send(successResult);
                }
                else {
...
export default { getCommissionType };