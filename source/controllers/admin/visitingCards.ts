...
const getVisitingCards = async (req: Request, res: Response, next: NextFunction) => {
    try {
        logging.info(NAMESPACE, 'Getting VisitingCards');
        var requiredFields = [];
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.length > 0) {
                    if (result[1] && result[1].length > 0) {
                        ...
                        let successResult = new ResultSuccess(200, true, 'Get Visiting Cards', result[1], result[0][0].totalCount);
                        return res.status(200).send(successResult);
                    }
                    else {
                        let successResult = new ResultSuccess(200, true, "Visiting Card Not Available", [], 0);
                        return res.status(200).send(successResult);
                    }
                } else {
                    let errorResult = new ResultError(400, true, "Error While Getting Visiting Cards", result, '');
                    next(errorResult);
                }
            } else {
        ...
        let errorResult = new ResultError(500, true, 'visitingCards.getVisitingCards() Exception', error, '');
        next(errorResult);
    }
};
...
const activeInactiveVisitingCard = async (req: Request, res: Response, next: NextFunction) => {
    try {
        logging.info(NAMESPACE, 'Active Inactive VisitingCard');
        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                let successResult = new ResultSuccess(200, true, 'Update VisitingCard', result, 1);
                return res.status(200).send(successResult);
        ...
        let errorResult = new ResultError(500, true, 'visitingCards.activeInactiveVisitingCard() Exception', error, '');
        next(errorResult);
    }
};
export default { getVisitingCards, insertVisitingCard, updateVisitingCard, activeInactiveVisitingCard }