...
const getPartnerUsersByRoleId = async (req: Request, res: Response, next: NextFunction) => {
    try {
        logging.info(NAMESPACE, 'Getting Partner User');
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.length > 0) {
                    if (result[0] && result[0].length > 0) {
                        let successResult = new ResultSuccess(200, true, 'Get Users', result[0], 1);
                        console.log(successResult);
                        return res.status(200).send(successResult);
                    } else {
                        let errorResult = new ResultSuccess(200, true, "Users Not Available", [], 0);
                        next(errorResult);
                    }
                } else {
                    let errorResult = new ResultError(400, true, "Error While Getting Users", result, '');
                    next(errorResult);
                }
            } else {
        ...
        let errorResult = new ResultError(500, true, 'Users.getPartnerUsers() Exception', error, '');
        next(errorResult);
    }
};
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.length > 0) {
                    if (result[1] && result[1].length > 0) {
                        let successResult = new ResultSuccess(200, true, 'Get Partners', result[1], result[0][0].totalCount);
                        console.log(successResult);
                        return res.status(200).send(successResult);
                    }
                    else {
                    ...
                    let errorResult = new ResultError(400, true, "Error While Getting Partners", result, '');
                    next(errorResult);
                }
            } else {
                let errorResult = new ResultError(401, true, "Unauthorized request", authorizationResult, '');
                ...
                if (result && result.length > 0 && result[0].length > 0) {
                    ...
                    if (result[0][0].message == "Contact No Already Exist") {
                        ...
                        if (accountNumber) {
                            ...
                            let bankResult = await query(bankSql);
                        }
                        //#region DSA Verify
...
export default { getPartnerUsersByRoleId, getPartnerList, deletePartnerByPartnerId, insertPartner, updatePartner, getPartnerDetailByPartnerId };
