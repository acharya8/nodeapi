...
const insertUpdateProfileDetail = async (req: Request, res: Response, next: NextFunction) => {
    try {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result) {
                    if (result.length > 0 && result[0].length > 0) {
                        let successResult = new ResultSuccess(200, true, 'Credit Card Profile Detail Saved', result[0], 1);
                        console.log(successResult);
                        return res.status(200).send(successResult);
                    } else {
                        let successResult = new ResultSuccess(200, true, 'Credit Card Profile Detail Saved', result, 1);
                        console.log(successResult);
                        return res.status(200).send(successResult);
                    }
                } else {
                    let errorResult = new ResultError(400, true, "Credit Card Profile Detail Not Saved", result, '');
                    next(errorResult);
                }
        ...
        let errorResult = new ResultError(500, true, 'loans.insertUpdateProfileDetail()', error, '');
        next(errorResult);
    }
};
        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (req.body.lastItr.includes("https:")) {
                    ...
                    if (result.length > 0 && result[0].length > 0) {
                        let successResult = new ResultSuccess(200, true, 'Credit Card Employment Detail Saved', result[0], 1);
                        console.log(successResult);
                        return res.status(200).send(successResult);
                    } else {
                        let errorResult = new ResultError(400, true, "Credit Card Employment Detail Not Saved", result, '');
                        next(errorResult);
                    }
                    ...
                    if (result && result.length > 0) {
                        if (result[0] && result[0].length > 0) {
                            ...
                            if (addressLength == (i + 1)) {
                                let successResult = new ResultSuccess(200, true, 'Crdit Card Address Saved', finalResult, finalResult.length);
                                console.log(successResult);
                                return res.status(200).send(successResult);
                            }
                        } else {
                            let errorResult = new ResultError(400, true, "Crdit Card Address Not Saved", new Error('Home Loan Customer Address Not Saved'), '');
                            next(errorResult);
                        }
                    } else {
                        let errorResult = new ResultError(400, true, "Crdit Card Address Not Saved", new Error('Home Loan Customer Address Not Saved'), '');
...
const insertUpdateCustomerWorkAddress = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.length > 0) {
                    if (result[0] && result[0].length > 0) {
                        let successResult = new ResultSuccess(200, true, 'Credit Card Customer Work Detatil Saved', result[0], 1);
                        console.log(successResult);
                        return res.status(200).send(successResult);
                    } else {
                        let successResult = new ResultSuccess(200, true, 'Credit Card Customer Work Detatil Saved', result, 1);
                        console.log(successResult);
                        return res.status(200).send(successResult);
                    }
                } else {
                    let errorResult = new ResultError(400, true, "Credit Card Customer Work Detatil Not Saved", result, '');
                    next(errorResult);
                }
            } else {
                let errorResult = new ResultError(401, true, "Unauthorized request", authorizationResult, '');
        ...
        let errorResult = new ResultError(500, true, 'creditCard.insertUpdateCustomerWorkAddress()', error, '');
        next(errorResult);
    }
};

const chooseCommunicationAddress = async (req: Request, res: Response, next: NextFunction) => {
    try {
        logging.info(NAMESPACE, 'Choose Communication Address');
        var requiredFields = [];
        var validationResult = header.validateRequiredFields(req, requiredFields);
        if (validationResult && validationResult.statusCode == 200) {
            let authorizationResult = await header.validateAuthorization(req, res, next);
                ...
                if (communicationselectioneResult && communicationselectioneResult.affectedRows >= 0) {
                    ...
                    if (statusResult && statusResult.length > 0) {
                        ...
                        if (pendingSeatus && pendingSeatus.length > 0) {
                            ...
                            let chageStatusResult = await query(chageStatusSql, customerCreditCardId);
...
const getIncompleteCreditCardDetail = async (req: Request, res: Response, next: NextFunction) => {
    try {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.length > 0 && result[0].length > 0) {
                    if (result[0][0].message && result[0][0].message == "Data Not Available") {
                        ...
                        let basicDetail: BasicDetailCCResponse = new BasicDetailCCResponse(obj[0].fullName, obj[0].birthdate, obj[0].panCardNo, obj[0].gender, obj[0].maritalStatusId, obj[0].email, obj[0].customerCreditCard.id, obj[0].customerCreditCard.otherCreditCardBankId, obj[0].customerCreditCard.maxCreditLimit, obj[0].customerCreditCard.availableCreditLimit, obj[0].customerCreditCard.isAlreadyCreditCard, obj[0].customerCreditCardEmploymentDetail ? obj[0].customerCreditCardEmploymentDetail.employmentTypeId : null, obj[0].customerCreditCardEmploymentDetail ? obj[0].customerCreditCardEmploymentDetail.bankId : null, obj[0].customerCreditCardEmploymentDetail ? obj[0].customerCreditCardEmploymentDetail.bankAccountNo : null, obj[0].customerCreditCardEmploymentDetail ? obj[0].customerCreditCardEmploymentDetail.lastItr : null, obj[0].customerCreditCardEmploymentDetail ? obj[0].customerCreditCardEmploymentDetail.educationTypeId : null, obj[0].customerCreditCardEmploymentDetail ? obj[0].customerCreditCardEmploymentDetail.officeContactNo : null, obj[0].customerCreditCardEmploymentDetail ? obj[0].customerCreditCardEmploymentDetail.id : null, obj[0].customerCreditCardEmploymentDetail ? obj[0].customerCreditCardEmploymentDetail.companyName : null, obj[0].customerCreditCardEmploymentDetail ? obj[0].customerCreditCardEmploymentDetail.professionName : null, obj[0].customerCreditCard ? obj[0].customerCreditCard.communicationAddressId : null, obj[0].id, obj[0].userId, obj[0].contactNo);
                        let permanentAddressDetail: CreditCardAddressResponse = new CreditCardAddressResponse(obj[0].label, obj[0].addressLine1, obj[0].addressLine2, obj[0].pincode, obj[0].cityId, obj[0].addressTypeId, obj[0].addressId);

                        let correspondenceAddressDetail: CreditCardAddressResponse = obj[0].correspondenseAddress ? new CreditCardAddressResponse(obj[0].correspondenseAddress.label, obj[0].correspondenseAddress.addressLine1, obj[0].correspondenseAddress.addressLine2, obj[0].correspondenseAddress.pincode, obj[0].correspondenseAddress.cityId, obj[0].correspondenseAddress.addressTypeId, obj[0].correspondenseAddress.addressId) : null;
                        let workAddressDetail: CreditCardAddressResponse = obj[0].workAddress ? new CreditCardAddressResponse(obj[0].workAddress.label, obj[0].workAddress.addressLine1, obj[0].workAddress.addressLine2, obj[0].workAddress.pincode, obj[0].workAddress.cityId, obj[0].workAddress.addressTypeId, obj[0].workAddress.addressId) : null;
                        let chooseCreditCardDetail: ChoosenCreditCardDetailResponse = obj[0].chooseCreditCardDetail ? new ChoosenCreditCardDetailResponse(obj[0].chooseCreditCardDetail.bankId, obj[0].chooseCreditCardDetail.bankCreditCardId, obj[0].chooseCreditCardDetail.referenceNo, obj[0].chooseCreditCardDetail.id, obj[0].chooseCreditCardDetail.bankName) : null;



                        let loanCompleteHistory: loanCompleteHistoryResponse = new loanCompleteHistoryResponse(obj[0].customerCreditCardCompleteHistory.isCompleted, obj[0].customerCreditCardCompleteHistory.completeScreen);

                        if (obj[0].customerCreditCardStatusHistory) {
                            loanStatus = new LoanStatusResponse(obj[0].customerCreditCardStatusHistory.id, obj[0].customerCreditCardStatusHistory.transactionDate, obj[0].customerCreditCardStatusHistory.creditCardStatus,
                                obj[0].customerCreditCardStatusHistory.isDataEditable, result[6][0].transactionDate, obj[0].customerCreditCard.displayName, null);
                        }


                        let response: CreditCardResponse = new CreditCardResponse(basicDetail, permanentAddressDetail, correspondenceAddressDetail, workAddressDetail, loanCompleteHistory, loanStatus, chooseCreditCardDetail);
...
const getCustomerCreditCardById = async (req: Request, res: Response, next: NextFunction) => {
    try {
        logging.info(NAMESPACE, 'Get Personal Loan Detail');
        ...
        let errorResult = new ResultError(500, true, 'creditCards.getIncompletePersonalLoanDetail()', error, '');
        next(errorResult);
    }
};

const creditCardEligibility = async (req: Request, res: Response, next: NextFunction) => {
    try {
        logging.info(NAMESPACE, 'Credit Card Eligibility');
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (checkResult && checkResult.length > 0) {
                    ...
                    if (checkResult[0].companyName) {
                        ...
                        let companyCatResult = await query(companyCatSql);
                    ...
                    if (result && result.length > 0) {
                        ...
                        let successResult = new ResultSuccess(200, true, 'Get Dsa', result.banks, result.banks.length);
                        return res.status(200).send(successResult);
                    } else {
                        let successResult = new ResultSuccess(200, true, 'Get Dsa', [], 0);
                        return res.status(200).send(successResult);
                    }
                }
            } else {
        ...
        let errorResult = new ResultError(500, true, 'creditCards.creditCardEligibility()', error, '');
        next(errorResult);
    }
}
        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.length > 0) {
                    ...
                    let errorResult = new ResultError(400, true, "Credit Card Customer Offer Not Saved", result, 0);
                    next(errorResult);
                }
            } else {
                let errorResult = new ResultError(401, true, "Unauthorized request", authorizationResult, '');
...
const getCustomerCreditCardRejectionReason = async (req: Request, res: Response, next: NextFunction) => {
    try {
        logging.info(NAMESPACE, 'get CreditCard Rejection Reason');
        var requiredFields = ['customerCreditCardId'];
        var validationResult = header.validateRequiredFields(req, requiredFields);
        if (validationResult && validationResult.statusCode == 200) {
            let authorizationResult = await header.validateAuthorization(req, res, next);
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.length > 0) {
                    ...
                    let successResult = new ResultSuccess(200, true, 'Get Credit Card Rejection Reason', result[0], 1);
                    console.log(successResult);
                    return res.status(200).send(successResult);
                } else {
                    let successResult = new ResultSuccess(200, true, 'Get Credit Card Rejection Reason', {}, 0);
                    console.log(successResult);
                    return res.status(200).send(successResult);
                }
        ...
        let errorResult = new ResultError(500, true, 'creditCards.getCustomerCreditCardRejectionReason() Exception', error, '');
        next(errorResult);
    }
};
const getCompletedCreditCards = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.length > 0) {
                    if (result[0] && result[0].length > 0) {
                        let successResult = new ResultSuccess(200, true, 'Customer Credit Cards', result[0], result[0].length);
                        console.log(successResult);
                        return res.status(200).send(successResult);
                    } else {
                    ...
                    let errorResult = new ResultError(400, true, "Error While Getting Customer Credit Cards", result, '');
                    next(errorResult);
                }
        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (pendingSeatus && pendingSeatus.length > 0) {
                    ...
                    let statusResult = await query(statusSql);
                    let delteSqlResult = await query("DELETE FROM customercreditcardoffer WHERE customerCreditCardId = ?", customerCreditCardId);
                    ...
                    if (chageStatusResult && chageStatusResult.affectedRows >= 0) {
                        let successResult = new ResultSuccess(200, true, 'Change Employment Type', chageStatusResult, 1);
                        return res.status(200).send(successResult);
                    }
                    else {
                        let errorResult = new ResultError(400, true, "creditCard.changeEmploymentType() Error", chageStatusResult, '');
                        next(errorResult);
                    }
