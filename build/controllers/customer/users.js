...
const roleResponse_1 = require("../../classes/output/customer/roleResponse");
const usersResponse_1 = require("../../classes/output/customer/usersResponse");
...
const clientVerifyContact = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        var requiredFields = ['contactNo'];
        var validationResult = apiHeader_1.default.validateRequiredFields(req, requiredFields);
        if (validationResult && validationResult.statusCode == 200) {
            logging_1.default.info(NAMESPACE, 'Customer Verify Contact No');
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (checkResult && checkResult.length == 0) {
                    ...
                    if (req.body.partNerId) {
                        let userId = yield query(`SELECT userId FROM partners WHERE id = ` + req.body.partNerId);
                    ...
                    if (result && result.length > 0) {
                        ...
                        if (result[1] && result[1].length > 0) {
                            if (req.body.partNerId) {
                                ...
                                let checkPartnerCustomerResult = yield query(checkPartnerCustomerSql);
                                if (!(checkPartnerCustomerResult && checkPartnerCustomerResult.length > 0)) {
                                    ...
                                    let partnerCustomeResult = yield query(partnerCustomeSql);
                                }
                            }
                            if (req.body.isReceiveMarketingCalls != undefined && req.body.isReceiveMarketingCalls != null) {
                                ...
                                let changeFlagResult = yield query(changeFlag);
                            }
                            user = new usersResponse_1.userResponse(result[1][0].userId, result[1][0].email, result[1][0].countryCode, result[1][0].password, result[1][0].profilePicUrl, result[1][0].isDisabled, result[1][0].roleId, result[1][0].id, result[1][0].temporaryCode, result[1][0].permanentCode, result[1][0].fullName, result[1][0].birthdate, result[1][0].gender, result[1][0].contactNo, result[1][0].alternativeContactNo, result[1][0].groupId, result[1][0].partnerId, result[1][0].panCardNo, result[1][0].aadhaarCardNo, result[1][0].isActive, result[1][0].isDelete, result[1][0].createdDate, result[1][0].modifiedDate, result[1][0].createdBy, result[1][0].modifiedBy, sessionToken, userRole, result[1][0].addressId, result[1][0].addressTypeId, result[1][0].label, result[1][0].addressLine1, result[1][0].addressLine2, result[1][0].pincode, result[1][0].cityId);
                    ...
                    let errorResult = new resulterror_1.ResultError(400, true, "ContactNo already exist try another contact", 'ContactNo already exist try another contact', '');
                    next(errorResult);
                }
            }
            else {
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'users.clientVerifyContact()', error, '');
        next(errorResult);
    }
});
...
const updateCustomerDetail = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.affectedRows >= 0) {
                    console.log(result);
                    let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Customer Updated', result, 1);
                    console.log(successResult);
                    return res.status(200).send(successResult);
                }
                else {
                    let errorResult = new resulterror_1.ResultError(400, true, "Customer Not Updated", result, '');
                    next(errorResult);
                }
            }
            else {
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'users.updateCustomerDetail()', error, '');
        next(errorResult);
    }
});
const updateCustomerProfile = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        var validationResult = apiHeader_1.default.validateRequiredFields(req, requiredFields);
        if (validationResult && validationResult.statusCode == 200) {
            logging_1.default.info(NAMESPACE, 'Update Customer Profile');
            let authorizationResult = yield apiHeader_1.default.validateAuthorization(req, res, next);
            if (authorizationResult.statusCode == 200) {
                let customerId = req.body.customerId;
                            ...
                            const delResp = yield S3.deleteObject({
                                ...
                                if (err) {
                                    ...
                                    if (result && result[0].length > 0) {
                                        console.log(result);
                                        let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Customer Updated', result[0], 1);
                                        console.log(successResult);
                                        return res.status(200).send(successResult);
                                    }
                                    else {
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'users.updateCustomerProfile()', error, '');
        next(errorResult);
    }
});
const insertRequestForBecomePartner = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'users.insertRequestForBecomePartner()', error, '');
        next(errorResult);
    }
});
const checkStatusForBecomeapartner = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        var requiredFields = ['customerId'];
        var validationResult = apiHeader_1.default.validateRequiredFields(req, requiredFields);
        if (validationResult && validationResult.statusCode == 200) {
            logging_1.default.info(NAMESPACE, 'Customer Get Non Disbursed Loan');
            let authorizationResult = yield apiHeader_1.default.validateAuthorization(req, res, next);
            if (authorizationResult.statusCode == 200) {
                let isAllow = false;
                let isLoanAvailable = false;
                let loans = yield query(`SELECT id FROM customerloans WHERE customerId = ?`, req.body.customerId);
                if (loans && loans.length > 0) {
                    ...
                    if (result && result[0].length > 0) {
                        ...
                        let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Loan Exist', data, 1);
                        return res.status(200).send(successResult);
                    }
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'users.checkStatusForBecomeapartner()', error, '');
        next(errorResult);
    }
});
exports.default = { clientVerifyContact, checkContactNoExist, validateSessionToken, updateCustomerDetail, updateCustomerProfile, insertRequestForBecomePartner, checkStatusForBecomeapartner };
