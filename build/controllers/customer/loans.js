...
const loanCompareResponse_1 = require("../../classes/output/loan/loanCompareResponse");
...
const insertUpdateCustomerLoan = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'loans.insertUpdateCustomerLoan()', error, '');
        next(errorResult);
    }
});
...
const getLoanCompareData = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        logging_1.default.info(NAMESPACE, 'Get Loan Compare Data By serviceId Id');
                ...
                if (result) {
                    ...
                    if (result && result.length > 0) {
                        for (let i = 0; i < result.length; i++) {
                            ...
                            let obj = new loanCompareResponse_1.LoanCompareResponse(result[i].bankName, emi, result[i].ROI, totalInterest, netPayableAmount);
                            sendData.push(obj);
                        }
                        let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Loan CompareDate', sendData, sendData.length);
                        console.log(successResult);
                        return res.status(200).send(successResult);
                    }
                    else {
                        let successResult = new resultsuccess_1.ResultSuccess(400, true, 'No Bank Policy Data Available', [], 0);
                        console.log(successResult);
                        return res.status(200).send(successResult);
                    }
                }
                else {
                    let errorResult = new resulterror_1.ResultError(400, true, "loans.getLoanCompareData() Error", result, '');
                    next(errorResult);
                }
            }
            else {
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'loans.getLoanCompareData()', error, '');
        next(errorResult);
    }
});
exports.default = { getCustomerLoansByStatusId, getCustomerLoansByStatusIdV2, getLoanCompareData };
