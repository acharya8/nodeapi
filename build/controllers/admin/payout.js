...
const getpartnerPayout = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        logging_1.default.info(NAMESPACE, 'Getting Partner Payout');
        ...
        if (authorizationResult.statusCode == 200) {
            ...
            if (result && result.length > 0) {
                if (result[1] && result[1].length > 0) {
                        ...
                        for (let i = 0; i < result[1].length; i++) {
                            ...
                            if (history && history.length > 0) {
                                for (let j = 0; j < history.length; j++) {
                                    let commissionSqlResult = yield query(`SELECT commission FROM bankloanpartnercommissions WHERE bankId = ` + history[j].bankId + ` AND serviceId = ` + history[j].serviceId + ` AND partnerId = ` + result[1][i].partnerId + ``);
                    ...
                    let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Get Partner Payout', result[1], length);
                    console.log(successResult);
                    return res.status(200).send(successResult);
                }
                else if (result[1] && result[1].length == 0) {
                    let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Get Partner Payout', result[1], result[0][0].totalCount);
                    console.log(successResult);
                    return res.status(200).send(successResult);
                }
            }
            else {
                let errorResult = new resulterror_1.ResultError(400, true, "Error While Getting Partner Payout", result, '');
                next(errorResult);
            }
        }
        else {
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'payout.getpartnerPayout() Exception', error, '');
        next(errorResult);
    }
});
const payoutRelease = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        logging_1.default.info(NAMESPACE, 'Insert Payout Release');
        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.affectedRows >= 0) {
                    ...
                    let lastInvoiceResult = yield query(lastInvoiceNumberSql);
                    ...
                    let pdfRes = yield printPdf(invoiceData);
                    if (pdfRes && pdfRes.pdfString) {
                        ...
                        yield S3.upload(params, (error, data) => __awaiter(void 0, void 0, void 0, function* () {
                            ...
                            try {
                                ...
                                let invoiceResult = yield query(sql);
                                if (invoiceResult) {
                                    data = {
                                        "invoiceUrl": data.Location
                                    };
                                    let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Insert Payout Release', data, 1);
                                    console.log(successResult);
                                    return res.status(200).send(successResult);
                                }
                            }
                            catch (error) {
                                let errorResult = new resulterror_1.ResultError(400, true, "Error While Inserting Partner Payout", result, '');
                                next(errorResult);
                            }
                        }));
                    }
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'payout.payoutRelease() Exception', error, '');
        next(errorResult);
    }
});
...
exports.default = { getpartnerPayout, payoutRelease };
