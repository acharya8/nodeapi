...
const getCreditCard = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        logging_1.default.info(NAMESPACE, 'Getting CreditCard');
        ...
        if (authorizationResult.statusCode == 200) {
            ...
            if (result && result[1].length >= 0) {
                if (result && result[1].length >= 0) {
                    let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Get CreditCard Successfully', result[1], result[0][0].totalCount);
                ...
                let errorResult = new resulterror_1.ResultError(400, true, "creditCard.getCreditCard() Error", result, '');
                next(errorResult);
            }
        }
        else {
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'customers.getCreditCard() Exception', error, '');
        next(errorResult);
    }
});
const insertUpdateCreditCard = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
                    ...
                    if (result.length > 0 && result[0].length > 0) {
                        ...
                        if (req.body.customerAddresses && req.body.customerAddresses.length > 0) {
                            ...
                            for (let i = 0; i < req.body.customerAddresses.length; i++) {
                                ...
                                let addressResult = yield query(sql);
                                if (addressResult && addressResult.length > 0) {
                                    if (addressResult[0] && addressResult[0].length > 0) {
                                        if (addressLength == (i + 1)) {
                                            let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Crdit Card Address Saved', addressResult, addressResult.length);
                                            if (addressTypeId == "3") {
                                    ...
                                    let errorResult = new resulterror_1.ResultError(400, true, "Credit Card  Detail Not Saved", result, '');
                                    next(errorResult);
                                }
                            }
                        }
                        ...
                        if (req.body.customerWorkAddresses) {
                            ...
                            let workAddressResult = yield query(workAddressSql);
                            if (workAddressResult && workAddressResult.length > 0) {
                                ...
                                let errorResult = new resulterror_1.ResultError(400, true, "Credit Card Customer Work Detatil Not Saved", workAddressResult, '');
                                next(errorResult);
                            }
                        }
                        ...
                        if (communicationselectioneResult && communicationselectioneResult.affectedRows >= 0) {
                            ...
                            if (statusResult && statusResult.length > 0) {
                                ...
                                if (pendingSeatus && pendingSeatus.length > 0) {
                                    ...
                                    let chageStatusResult = yield query(chageStatusSql, creditCardId);
...
const getCreditCardById = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        logging_1.default.info(NAMESPACE, 'Getting CreditCard');
        let authorizationResult = yield apiHeader_1.default.validateAuthorization(req, res, next);
        if (authorizationResult.statusCode == 200) {
            let customerId = yield query("select customerId from customercreditcards WHERE id = ?", req.body.customerCreditCardId);
            ...
            if (result && result[0].length > 0) {
                ...
                let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Get CreditCard Successfully', cusotmerCreditCard, 1);
                return res.status(200).send(successResult);
            }
            else {
                let errorResult = new resulterror_1.ResultError(400, true, "creditCard.getCreditCard() Error", result, '');
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'creditcard.getCreditCard() Exception', error, '');
        next(errorResult);
    }
});
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.length > 0) {
                    if (result[0] && result[0].length > 0) {
                        ...
                        let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Credit Card Customer Offer Saved', result, 1);
                        return res.status(200).send(successResult);
                    }
                }
                else {
                    let errorResult = new resulterror_1.ResultError(400, true, "Credit Card Customer Offer Not Saved", result, '');
                    next(errorResult);
                }
            }
            else {
...
const changeCreditCardOfferStatus = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        logging_1.default.info(NAMESPACE, 'Change Credit Card Offer Status');
        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                let insertStatusHistoryResutlt = yield query(insertStatusHistorySql);
                if (insertStatusHistoryResutlt && insertStatusHistoryResutlt.affectedRows > 0) {
                    ...
                    let updateResutlt = yield query(updateSql);
                    if (updateResutlt && updateResutlt.affectedRows > 0) {
                        ...
                        let status = yield query(`SELECT status FROM creditcardstatuses WHERE id = ?`, creditCardStatusId);
                        let title = "CreditCard Status Change to " + status[0].status + "";
                        let description = "CreditCard Status Change to " + status[0].status + "";
                        let creditCardStatus = yield query(`SELECT creditcardstatuses.status FROM creditcardstatuses INNER JOIN customercreditcards ON customercreditcards.statusId = creditcardstatuses.id WHERE creditcardstatuses.id = ?`, creditCardStatusId);
                        let dataBody = {
                            type: 7,
                        ...
                        VALUES(` + customerUserId + `, 7, '` + title + `', '` + description + `', '` + JSON.stringify(dataBody) + `', null, ` + authorizationResult.currentUser.id + `, ` + authorizationResult.currentUser.id + `)`;
                            ...
                            yield notifications_1.default.sendMultipleNotification([customerFcm], 7, customerCreditCardId, title, description, '', null, null, null, null, customerCreditCardId, creditCardStatus[0].status);
                        }
                        if (partnerFcm) {
                            let notificationSql = `INSERT INTO usersnotification(userId, notificationType, title, message, bodyJson, imgUrl, createdBy, modifiedBy) 
                        VALUES(` + partnerUserId + `, 7, '` + title + `', '` + description + `', '` + JSON.stringify(dataBody) + `', null, ` + authorizationResult.currentUser.id + `, ` + authorizationResult.currentUser.id + `)`;
                            let notificationResult = yield query(notificationSql);
                            yield notifications_1.default.sendMultipleNotification([partnerFcm], 7, customerCreditCardId, title, description, '', null, null, null, null, customerCreditCardId, creditCardStatus[0].status);
                        }
                        //#endregion Notification
                        let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Customer Credit Card Offer Status Change', insertStatusHistoryResutlt, 1);
                        return res.status(200).send(successResult);
                    }
                    else {
                        let errorResult = new resulterror_1.ResultError(400, true, "creditCards.changeCreditCardOfferStatus() Error", new Error('Error While Change Status'), '');
                        next(errorResult);
                    }
                }
                else {
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'creditCards.changeCreditCardOfferStatus()', error, '');
        next(errorResult);
    }
});
const inserUpdaterejectCreditCardOffer = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        logging_1.default.info(NAMESPACE, 'Customer Credit Card Reject');
        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (id) {
                    ...
                    let updateStatusHistoryResult = yield query(updateStatusHistorySql);
                    if (updateStatusHistoryResult && updateStatusHistoryResult.affectedRows > 0) {
                        if (req.body.reasons && req.body.reasons.length > 0) {
                            let deleteQuery = yield query("DELETE FROM reasons WHERE customerCreditCardId = ?", customerCreditCardId);
                        ...
                        if (customerFcm) {
                            ...
                            let notificationResult = yield query(notificationSql);
                            yield notifications_1.default.sendMultipleNotification([customerFcm], 8, customerCreditCardId, title, description, '', null, null, null, null, customerCreditCardId, null);
                        }
                        if (partnerFcm) {
                            let notificationSql = `INSERT INTO usersnotification(userId, notificationType, title, message, bodyJson, imgUrl, createdBy, modifiedBy) 
                            ...
                            let notificationResult = yield query(notificationSql);
                            yield notifications_1.default.sendMultipleNotification([partnerFcm], 8, customerCreditCardId, title, description, '', null, null, null, null, customerCreditCardId, null);
                        }
                        //#endregion Notification
                        let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Customer Credit Card Offer Status Change', updateStatusHistoryResult, 1);
                        return res.status(200).send(successResult);
                    }
                    else {
                        let errorResult = new resulterror_1.ResultError(400, true, "creditCards.inserUpdaterejectCreditCardOffer() Error", new Error('Error While Change Status'), '');
                        next(errorResult);
                    }
                }
                else {
                    ...
                    if (insertStatusHistoryResutlt && insertStatusHistoryResutlt.affectedRows > 0) {
                        ...
                        if (updateResutlt && updateResutlt.affectedRows > 0) {
                            ...
                            let rejectionResult = yield query(sql);
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'creditCards.inserUpdaterejectCreditCardOffer()', error, '');
        next(errorResult);
    }
});
const getCreditCardStatuses = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        logging_1.default.info(NAMESPACE, 'Getting Credit Card Statuses');
        let authorizationResult = yield apiHeader_1.default.validateAuthorization(req, res, next);
        if (authorizationResult.statusCode == 200) {
            ...
            if (result && result.length > 0) {
                if (result[0] && result[0].length > 0) {
                    let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Get Credit Card Status Successfully', result[0], result[0].length);
                    return res.status(200).send(successResult);
                }
                else if (result[0] && result[0].length == 0) {
                    let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Get Credit Card Status Successfully', [], result[0].length);
                    return res.status(200).send(successResult);
                }
                else {
                    let errorResult = new resulterror_1.ResultError(400, true, 'Data Not Available', new Error('Data Not Available'), '');
                ...
                let errorResult = new resulterror_1.ResultError(400, true, "creditCard.getCreditCardStatuses() Error", result, '');
                next(errorResult);
            }
        }
        else {
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'creditCard.getCreditCardStatuses() Exception', error, '');
        next(errorResult);
    }
});
exports.default = { getCreditCard, insertUpdateCreditCard, getCreditCardById, insertUpdateCustomerCreditCardOffer, changeCreditCardOfferStatus, inserUpdaterejectCreditCardOffer, getCreditCardStatuses };
