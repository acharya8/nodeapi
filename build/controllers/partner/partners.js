                                    ...
                                    if (cnt == req.body.documents.length) {
                                        ...
                                        if (referCoinResult && referCoinResult.length > 0) {
                                            ...
                                            if (referCoinResult[0].isScratchCard) {
                                                ...
                                                let userWalletId;
                                                let userWalletIdResult = yield query(`SELECT id,coin FROM userwallet WHERE userId = ?`, result[3][0].userId);
                                                if (userWalletIdResult && userWalletIdResult.length > 0) {
                                                    ...
                                                    let insertWalletAmount = yield query(`INSERT INTO userwallet (userId,coin,createdBy,modifiedBy) VALUES (` + result[3][0].userId + `,` + referCoin + `,` + authorizationResult.currentUser.id + `,` + authorizationResult.currentUser.id + `)`);
...
const getNetworkandTeamPartnerListByRoleId = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        logging_1.default.info(NAMESPACE, 'Getting Partner List Under DSA');
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.length > 0) {
                    if (result[0] && result[0].length > 0) {
                        let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Get Partners', result[0], result[0].length);
                        return res.status(200).send(successResult);
                    }
                    else {
                    ...
                    let errorResult = new resulterror_1.ResultError(400, true, "partners.insertPartner() Error", result, '');
                    next(errorResult);
                }
            }
            else {
...
const getNetworkandTeamPartnerHierarchyListByRoleId = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        logging_1.default.info(NAMESPACE, 'Getting Partner List Under DSA');
        var requiredFields = ['partnerId', 'roleIds'];
        var validationResult = apiHeader_1.default.validateRequiredFields(req, requiredFields);
                    ...
                    if (result[0] && result[0].length > 0) {
                        ...
                        if (subDsa && subDsa.length > 0) {
                            for (let i = 0; i < subDsa.length; i++) {
                                ...
                                let subDsaResult = yield query(subDsaSql);
                        ...
                        result = [data];
                        let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Get Partners', result, 1);
                        return res.status(200).send(successResult);
                    }
                    else {
                        let successResult = new resultsuccess_1.ResultSuccess(200, true, 'No Data Available', [], 0);
...
const getNetworkandTeamPartnerListByParentPartnerId = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        logging_1.default.info(NAMESPACE, 'Getting Partner List Under DSA');
        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.length > 0) {
                    ...
                    let errorResult = new resulterror_1.ResultError(400, true, "partners.getNetworkandTeamPartnerListByParentPartnerId() Error", result, '');
                    next(errorResult);
                }
            }
            else {
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'partners.getNetworkandTeamPartnerListByParentPartnerId() Exception', error, '');
        next(errorResult);
    }
});
...
const verifyPartner = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        logging_1.default.info(NAMESPACE, 'Verifying Partner');
        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.affectedRows >= 0) {
                    ...
                    let temporaryCodeResult = yield query(temporaryCodeSql, req.body.userId);
                    let permanentCode = '';
                    ...
                    let generatePermanentCodeRes = yield query(generatePermanentCodeSQL);
                    //#region Notification
                    let partnerFcm = "";
                    ...
                    if (partnerFcm) {
                        ...
                        yield notifications_1.default.sendMultipleNotification([partnerFcm], 9, partnerUserId, title, description, '', null, null, null, null, null, null);
                    }
                    //#endregion Notification
                    let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Verify Partner Successfully', result, 1);
                    console.log(successResult);
                    return res.status(200).send(successResult);
                }
                else {
                    let errorResult = new resulterror_1.ResultError(400, true, "partners.getPartnerDetailByPartnerId() Error", new Error("Error During Verifying Partner"), '');
                    next(errorResult);
                }
            }
            else {
...
const updatePartnerProfile = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        var requiredFields = ['partnerId', 'fullName', 'contactNo', 'panCardNo', 'aadhaarCardNo'];
        var validationResult = apiHeader_1.default.validateRequiredFields(req, requiredFields);
        if (validationResult && validationResult.statusCode == 200) {
            logging_1.default.info(NAMESPACE, 'Update Partner Profile');
            let authorizationResult = yield apiHeader_1.default.validateAuthorization(req, res, next);
            if (authorizationResult.statusCode == 200) {
                let partnerId = req.body.partnerId;
                let fullName = req.body.fullName;
                                    ...
                                    const delResp = yield S3.deleteObject({
                                        ...
                                        if (err) {
                                            ...
                                            if (result && result[0].length > 0) {
                                                ...
                                                if (result[4] && result[4].length > 0) {
                                                    parentPartner = new partnerResponse_1.partnerResponse(result[4][0].id, result[4][0].parentParnerId, result[4][0].userId, result[4][0].temporaryCode, result[4][0].permanentCode, result[4][0].fullName, result[4][0].gender, result[4][0].contactNo, result[4][0].aadhaarCardNo, result[4][0].panCardNo, result[4][0].cityId, result[4][0].companyName, result[4][0].companyTypeId, result[4][0].udhyamAadhaarNo, result[4][0].companyRegNo, result[4][0].professionTypeId, result[4][0].workExperience, result[4][0].haveOffice, result[4][0].businessName, result[4][0].businessAddress, result[4][0].gstNo, result[4][0].commitment, result[4][0].designationId, result[4][0].referralCode, result[4][0].isActive, result[4][0].isDelete, result[4][0].createdDate, result[4][0].modifiedDate, result[4][0].createdBy, result[4][0].modifiedBy);
                                                }
                                                if (result[1] && result[1].length > 0) {
                                                    let address;
                                                ...
                                                let errorResult = new resulterror_1.ResultError(400, true, "Partner Not Updated", result, '');
                                                next(errorResult);
                                            }
                                        }
                                    }));
                                                ...
                                                yield S3.upload(params, (error, data) => __awaiter(void 0, void 0, void 0, function* () {
                                                    ...
                                                    if (result && result[0].length > 0) {
                                                        ...
                                                        if (result[1] && result[1].length > 0) {
                                                            ...
                                                            let partnerBadgeSql = "SELECT * FROM badges WHERE id = " + result[5][0].currentBadgeId;
                                                            let partnerBadgeResult = yield query(partnerBadgeSql);
                                                            user = new usersResponse_1.userResponse(result[1][0].userId, result[1][0].email, result[1][0].countryCode, result[1][0].password, result[1][0].profilePicUrl, result[1][0].isDisabled, result[1][0].roleId, result[1][0].id, result[1][0].parentPartnerId, result[1][0].temporaryCode, result[1][0].permanentCode, result[1][0].fullName, result[1][0].gender, result[1][0].contactNo, result[1][0].aadhaarCardNo, result[1][0].panCardNo, result[1][0].cityId, result[1][0].companyName, result[1][0].companyTypeId, result[1][0].udhyamAadhaarNo, result[1][0].companyRegNo, result[1][0].professionTypeId, result[1][0].workExperience, result[1][0].haveOffice, result[1][0].businessName, result[1][0].businessAddress, result[1][0].gstNo, result[1][0].commitment, result[1][0].designationId, result[1][0].referralCode, result[1][0].isActive, result[1][0].isDelete, result[1][0].createdDate, result[1][0].modifiedDate, result[1][0].createdBy, result[1][0].modifiedBy, result[1][0].currentBadgeId, (partnerBadgeResult && partnerBadgeResult.length > 0 ? partnerBadgeResult[0].name : ""), null, userRole, parentPartner, (address && address.id), (address && address.label), (address && address.addressLine1), (address && address.addressLine2), (address && address.pincode), (address && address.city), (address && address.district), (address && address.state), (address && address.districtId), (education && education.id), (address && address.stateId), (education && education.educationTypeId), (education && education.instituteName), (education && education.passingYear), (education && education.resume), result[1][0].addressLine1, result[1][0].addressLine2, result[1][0].pincode, result[1][0].workAddressCityId, result[1][0].jobType);
                                                        }
                                                        let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Login SuccessFully', [user], 1);
                                                        console.log(successResult);
                                                        return res.status(200).send(successResult);
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'partners.updatePartnerProfile()', error, '');
        next(errorResult);
    }
});
const getCommisionListByPartnerId = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        var requiredFields = ['partnerId'];
        var validationResult = apiHeader_1.default.validateRequiredFields(req, requiredFields);
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.length > 0) {
                    if (result[0] && result[0].length > 0) {
                        let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Get Partner Commission List', result[0], result[0].length);
                        return res.status(200).send(successResult);
                    }
                    else {
                        let errorResult = new resulterror_1.ResultError(400, true, "Data Not Available", [], '');
                    ...
                    let errorResult = new resulterror_1.ResultError(400, true, "partners.getCommisionListByPartnerId() Error", result, '');
                    next(errorResult);
                }
            }
            else {
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'partners.getCommisionListByPartnerId()', error, '');
        next(errorResult);
    }
});
const insertUpdatePartnerBankDetail = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        logging_1.default.info(NAMESPACE, 'Insert Partner Bank Detail');
        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.length > 0) {
                    if (result[0] && result[0].length > 0) {
                        let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Insert Partner Bank Detail', result[0], result[0].length);
                        return res.status(200).send(successResult);
                    }
                    else {
                        let errorResult = new resulterror_1.ResultError(400, true, "Data Not Available", [], '');
                    ...
                    let errorResult = new resulterror_1.ResultError(400, true, "partners.insertPartnerBankDetail() Error", result, '');
                    next(errorResult);
                }
            }
            else {
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'partners.insertPartnerBankDetail() Exception', error, '');
        next(errorResult);
    }
});
const getBankDetailByPartnerId = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        var requiredFields = ['partnerId'];
        var validationResult = apiHeader_1.default.validateRequiredFields(req, requiredFields);
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.length > 0) {
                    if (result[0] && result[0].length >= 0) {
                        let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Get Partner Bank Detail', result[0], result[0].length);
                        return res.status(200).send(successResult);
                    }
                    else {
                        let errorResult = new resulterror_1.ResultError(400, true, "Data Not Available", [], '');
                    ...
                    let errorResult = new resulterror_1.ResultError(400, true, "partners.getBankDetailByPartnerId() Error", result, '');
                    next(errorResult);
                }
            }
            else {
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'partners.getBankDetailByPartnerId()', error, '');
        next(errorResult);
    }
});
...
const checkContactNoExistWithServiceId = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
                        ...
                        for (let i = 0; i < loans.length; i++) {
                            if (loans[i].partnerId != partnerId) {
                                ...
                                if (days < daysResult[0].value) {
                                    let errorResult = new resulterror_1.ResultError(402, true, '', new Error("This Contact Number has already apply for Loan with another partner so,you can't use this contact number "), '');
                                    next(errorResult);
                                    break;
                                }
                                else {
                                    let updateQueryResult = yield query(`UPDATE partnercustomers SET partnerId = ?,modifiedDate = CURRENT_TIMESTAMP(),modifiedBy = ` + authorizationResult.currentUser.id + ``, partnerId);
                                }
                            }
                            else {
...
const checkContactNoExistForCreditCard = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        var requiredFields = ['contactNo'];
        var validationResult = apiHeader_1.default.validateRequiredFields(req, requiredFields);
        if (validationResult && validationResult.statusCode == 200) {
            logging_1.default.info(NAMESPACE, 'Customer Contact No Exist For Credit Card');
            let contactNo = req.body.contactNo;
            let authorizationResult = yield apiHeader_1.default.validateAuthorization(req, res, next);
            if (authorizationResult.statusCode == 200) {
                let contactNo = req.body.contactNo;
...
const checkContactNoExistWithServiceIdFroOtherLoansAndServices = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        var requiredFields = ['contactNo', 'serviceId'];
        var validationResult = apiHeader_1.default.validateRequiredFields(req, requiredFields);
        if (validationResult && validationResult.statusCode == 200) {
...
const getPartnerGroupCustomers = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        var requiredFields = ['partnerId'];
        var validationResult = apiHeader_1.default.validateRequiredFields(req, requiredFields);
        if (validationResult && validationResult.statusCode == 200) {
            logging_1.default.info(NAMESPACE, 'Getting Partner Customers');
            let authorizationResult = yield apiHeader_1.default.validateAuthorization(req, res, next);
            if (authorizationResult.statusCode == 200) {
                let partnerId = req.body.partnerId;
                let startIndex = req.body.startIndex ? req.body.startIndex : 0;
                ...
                if (result && result.length > 0) {
                    if (result[0] && result[0].length > 0) {
                        let successResult = new resultsuccess_1.ResultSuccess(200, true, 'Get Partner Customers', result[0], result[0].length);
                        return res.status(200).send(successResult);
                    }
                    else {
                        let successResult = new resultsuccess_1.ResultSuccess(200, true, 'No Data Available', [], 0);
                    ...
                    let errorResult = new resulterror_1.ResultError(400, true, "partners.getPartnerGroupCustomers() Error", result, '');
                    next(errorResult);
                }
            }
            else {
        ...
        let errorResult = new resulterror_1.ResultError(500, true, 'users.getPartnerGroupCustomers()', error, '');
        next(errorResult);
    }
});
